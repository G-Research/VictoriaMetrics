...
func (s *ReadOnlySearch) Init(qt *querytracer.Tracer, storage *ReadOnlyStorage, tfss []*TagFilters, tr TimeRange, maxMetrics int, deadline uint64) int {
	...
	var err error
	s.idb, err = openReadOnlyCurrentIndexDB(idbPath, storage)
	if err != nil {
		return 0
	}
	...
	if err := func() (err error) {
		...
		s.err = fmt.Errorf("cannot initialize search for tagFilters=%s on the time range %s: %w", tfss, tr.String(), err)
	}
